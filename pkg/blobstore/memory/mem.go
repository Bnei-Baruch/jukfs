...
// Package memory registers the "memory" blobstore storage type, storing blobs
// in an in-memory map.
package memory // import "github.com/Bnei-Baruch/jukfs/pkg/blobstore/memory"
...
import (
	...
	"github.com/Bnei-Baruch/jukfs/internal/lru"
)

// Storage is an in-memory implementation of the blobstore Storage
...
var _ blobstore.BlobStreamer = (*Storage)(nil)

func init() {
	blobstore.RegisterStorageConstructor("memory", blobstore.StorageConstructor(newFromConfig))
}

func newFromConfig(_ blobstore.Loader, config jsonconfig.Obj) (blobstore.Storage, error) {
...
func (s *Storage) ReceiveBlob(ctx context.Context, br blob.Ref, source io.Reader) (blob.SizedRef, error) {
	...
	if !br.HashMatches(h) {
		...
		// blobstore.Receive now does it. But for testing code,
		// it's worth the cost.
...
func (s *Storage) StreamBlobs(ctx context.Context, dest chan<- blobstore.BlobAndToken, contToken string) error {
	...
	for _, br := range sorted {
		...
		case <-ctx.Done():
			return ctx.Err()
		case dest <- blobstore.BlobAndToken{
...
// See comment in https://camlistore-review.googlesource.com/#/c/3986/24/pkg/blobstore/localdisk/localdisk.go
