...
/*
Package shard registers the "shard" blobstore storage type,
...
*/
package shard // import "github.com/Bnei-Baruch/jukfs/pkg/blobstore/shard"
...
func newFromConfig(ld blobstore.Loader, config jsonconfig.Obj) (storage blobstore.Storage, err error) {
	...
	sto.shards = make([]blobstore.Storage, len(sto.shardPrefixes))
...
func (sto *shardStorage) shard(b blob.Ref) blobstore.Storage {
...
func (sto *shardStorage) batchedShards(ctx context.Context, blobs []blob.Ref, fn func(blobstore.Storage, []blob.Ref) error) error {
...
func (sto *shardStorage) RemoveBlobs(ctx context.Context, blobs []blob.Ref) error {
	return sto.batchedShards(context.TODO(), blobs, func(s blobstore.Storage, blobs []blob.Ref) error {
...
func (sto *shardStorage) StatBlobs(ctx context.Context, blobs []blob.Ref, fn func(blob.SizedRef) error) error {
	...
	// here.
	return sto.batchedShards(ctx, blobs, func(s blobstore.Storage, blobs []blob.Ref) error {
...
func (sto *shardStorage) EnumerateBlobs(ctx context.Context, dest chan<- blob.SizedRef, after string, limit int) error {
	return blobstore.MergedEnumerateStorage(ctx, dest, sto.shards, after, limit)
}

func init() {
	blobstore.RegisterStorageConstructor("shard", blobstore.StorageConstructor(newFromConfig))
}
