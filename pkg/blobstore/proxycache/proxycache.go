...
/*
Package proxycache registers the "proxycache" blobstore storage type,
which uses a provided blobstore as a cache for a second origin
blobstore.
...
*/
package proxycache // import "github.com/Bnei-Baruch/jukfs/pkg/blobstore/proxycache"
...
type Storage struct {
	origin blobstore.Storage
	cache  blobstore.Storage
...
var (
	_ blobstore.Storage = (*Storage)(nil)
	_ blob.SubFetcher    = (*Storage)(nil)
	// TODO:
	// _ blobstore.Generationer = (*Storage)(nil)
)
...
func New(maxBytes int64, cache, origin blobstore.Storage) *Storage {
	sto := &Storage{
		origin:        origin,
		cache:         cache,
...
func init() {
	blobstore.RegisterStorageConstructor("proxycache", blobstore.StorageConstructor(newFromConfig))
}

func newFromConfig(ld blobstore.Loader, config jsonconfig.Obj) (blobstore.Storage, error) {
	var (
...
func (sto *Storage) Fetch(ctx context.Context, b blob.Ref) (rc io.ReadCloser, size uint32, err error) {
	...
	if _, err := blobstore.Receive(ctx, sto.cache, b, bytes.NewReader(all)); err != nil {
