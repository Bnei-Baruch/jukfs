...
package storagetest // import "github.com/Bnei-Baruch/jukfs/pkg/blobstore/storagetest"
...
type Opts struct {
	...
	New func(*testing.T) (sto blobstore.Storage, cleanup func())
...
func Test(t *testing.T, fn func(*testing.T) (sto blobstore.Storage, cleanup func())) {
...
func TestOpt(t *testing.T, opt Opts) {
	...
	t.Logf("Testing blobstore storage %T", sto)
...
func CheckEnumerate(sto blobstore.Storage, wantUnsorted []blob.SizedRef, opts ...interface{}) error {
	var after string
	var n = 1000
...
func testStat(t *testing.T, sto blobstore.BlobStatter, blobs []blob.Ref, want []blob.SizedRef) {
...
// consistency.
func TestStreamer(t *testing.T, bs blobstore.BlobStreamer, opts ...StreamerTestOpt) {

	var sawEnum map[blob.SizedRef]bool
	if enumer, ok := bs.(blobstore.BlobEnumerator); ok {
		...
		defer cancel()
		if err := blobstore.EnumerateAll(enumCtx, enumer, func(sb blob.SizedRef) error {
			sawEnum[sb] = true
			return nil
		}); err != nil {
	...
	ch := make(chan blobstore.BlobAndToken)
