...
import (
	...
	"github.com/Bnei-Baruch/jukfs/pkg/blobstore/storagetest"
...
func mustReceive(t *testing.T, dst blobstore.Storage, tb *test.Blob) blob.SizedRef {
	tbRef := tb.BlobRef()
	sb, err := blobstore.Receive(ctxbg, dst, tbRef, tb.Reader())
	if err != nil {
...
func TestReceiveGood(t *testing.T) {
	...
	for i, rep := range sto.replicas {
		got, err := blobstore.StatBlob(ctxbg, rep, sb.Ref)
		if err != nil {
