...
/*
Package replica registers the "replica" blobstore storage type,
...
*/
package replica // import "github.com/Bnei-Baruch/jukfs/pkg/blobstore/replica"
...
var (
	_ blobstore.Generationer    = (*replicaStorage)(nil)
	_ blobstore.WholeRefFetcher = (*replicaStorage)(nil)
)
...
type replicaStorage struct {
	...
	replicas        []blobstore.Storage

	// Replicas for reading:
	readPrefixes []string
	readReplicas []blobstore.Storage
...
func NewForTest(sto []blobstore.Storage) blobstore.Storage {
	sto = append([]blobstore.Storage(nil), sto...) // clone
...
func newFromConfig(ld blobstore.Loader, config jsonconfig.Obj) (storage blobstore.Storage, err error) {
	sto := &replicaStorage{
	...
	for _, prefix := range sto.replicaPrefixes {
		...
		if err != nil {
			...
			var ok bool
			if s, ok = h.(blobstore.Storage); !ok {
				return nil, err
			}
		}
...
func (sto *replicaStorage) ReceiveBlob(ctx context.Context, br blob.Ref, src io.Reader) (_ blob.SizedRef, err error) {
	...
	uploadToReplica := func(idx int, dst blobstore.BlobReceiver) {
		// Using ReceiveNoHash because it's already been
		// verified implicitly by the io.Copy above:
		sb, err := blobstore.ReceiveNoHash(ctx, dst, br, bytes.NewReader(buf.Bytes()))
...
func (sto *replicaStorage) RemoveBlobs(ctx context.Context, blobs []blob.Ref) error {
	...
	removeFrom := func(s blobstore.Storage) {
	...
	if nSuccess > 0 {
		...
		// effort and we return nil if any of the blobstores said
...
func (sto *replicaStorage) EnumerateBlobs(ctx context.Context, dest chan<- blob.SizedRef, after string, limit int) error {
	return blobstore.MergedEnumerateStorage(ctx, dest, sto.readReplicas, after, limit)
}
...
func (sto *replicaStorage) ResetStorageGeneration() error {
	...
	n := 0
	for _, replica := range sto.replicas {
		if g, ok := replica.(blobstore.Generationer); ok {
			n++
...
func (sto *replicaStorage) OpenWholeRef(wholeRef blob.Ref, offset int64) (rc io.ReadCloser, wholeSize int64, err error) {
	// TODO: race these? first to respond?
	for _, replica := range sto.readReplicas {
		if v, ok := replica.(blobstore.WholeRefFetcher); ok {
...
func init() {
	blobstore.RegisterStorageConstructor("replica", blobstore.StorageConstructor(newFromConfig))
}
