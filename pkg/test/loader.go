...
type Loader struct {
	mu  sync.Mutex
	sto map[string]blobstore.Storage
}

var _ blobstore.Loader = (*Loader)(nil)
...
func (ld *Loader) SetStorage(prefix string, s blobstore.Storage) {
	...
	defer ld.mu.Unlock()
	if ld.sto == nil {
		ld.sto = make(map[string]blobstore.Storage)
	}
	ld.sto[prefix] = s
}
...
func (ld *Loader) genStorage(prefix string) (blobstore.Storage, error) {
